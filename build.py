...
PHONE = DATA['phone']
PHONE_HREF = '+1' + ''.join(ch for ch in PHONE if ch.isdigit())
...
SERVICE_DESCRIPTIONS = {
    ...
    'Equipment Repair & Replacement': 'Yes, Dixon Pool Service can help to repair or replace any filtration equipment. (ie. Pumps, Filters, Heaters, Pool lights, cleaners etc.)',
...
validate_pair(BEFORE_AFTER_PAIR)
BEFORE_AFTER_ASSETS = {
    role: PIPELINE_ROOT / BEFORE_AFTER_PAIR[role]['local_path']
    for role in ('before', 'after')
}

SOURCE_PAGE_INVENTORY = {
    'Home': 'index.html',
    'About': 'about.html',
    'Services': 'services.html',
    'Contact': 'contact.html',
}
REQUIRED_STRUCTURAL_PAGES = {
    'about.html', 'contact.html', 'privacy-policy.html', 'terms-of-service.html'
}
AUTHORIZED_ADDITIONAL_PAGES = {'faq.html'}

# Cycle 1's repair authorization carries the approved, byte-identical captured
# business hero into every redesign subpage. This is preview continuity, not a
# claim that the original source used the same image on every source subpage.
PAGE_HERO_MAP = {
    name: DATA['images'][0]['local_path']
    for name in (
        'about.html', 'services.html', 'faq.html', 'contact.html',
...
PAGES = {
    ...
    'terms-of-service.html': {
        'label': 'Terms',
        'title': 'Terms of Service | Dixon Pool Service Concept Redesign',
        'description': 'Terms of service for the Dixon Pool Service concept redesign preview.',
        'footer_only': True,
    },
}
...
.comparison{max-width:900px;margin:0 auto}.comparison-stage{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.75rem}.comparison-stage img{width:100%;height:auto;aspect-ratio:4/3;object-fit:cover;border-radius:var(--radius);box-shadow:var(--shadow)}.comparison-label{display:block;margin:.45rem 0 0;color:var(--navy);font-weight:900}.comparison-control{display:none}.comparison.is-enhanced .comparison-stage{display:block;position:relative;overflow:hidden;border-radius:var(--radius);box-shadow:var(--shadow);aspect-ratio:4/3;background:var(--navy)}.comparison.is-enhanced .comparison-stage>div{position:absolute;inset:0}.comparison.is-enhanced .comparison-stage img{position:absolute;inset:0;width:100%;height:100%;border-radius:0;box-shadow:none}.comparison.is-enhanced .comparison-after{clip-path:inset(0 calc(100% - var(--comparison-position,50%)) 0 0)}.comparison.is-enhanced .comparison-label{position:absolute;top:1rem;z-index:2;margin:0;padding:.35rem .65rem;border-radius:999px;background:rgba(7,31,43,.82);color:#fff}.comparison-label-before{right:1rem}.comparison-label-after{left:1rem}.comparison.is-enhanced .comparison-control{display:grid;gap:.35rem;margin-top:1rem}.comparison-control label{font-weight:800;color:var(--navy)}.comparison-control input{width:100%;accent-color:var(--primary)}.faq-list{display:grid;gap:1rem}.faq-list details{background:#fff;border:1px solid var(--line);border-radius:14px;padding:1rem 1.2rem;box-shadow:0 6px 18px rgba(16,86,130,.06)}.faq-list summary{cursor:pointer;color:var(--navy);font-weight:900}.faq-list p{margin:.8rem 0 .2rem}.legal{max-width:820px}.legal h2{color:var(--navy);margin-top:2rem}
...
def page_hero(filename: str, eyebrow: str, title: str, description: str) -> str:
    image='assets/'+Path(PAGE_HERO_MAP[filename]).name
...
def comparison() -> str:
    return render_before_after(
        BEFORE_AFTER_PAIR,
        before_href='assets/pool-before.jpg',
        after_href='assets/pool-after.jpg',
        before_alt='Pool before Dixon Pool Service care',
        after_alt='Pool after Dixon Pool Service care',
        input_id='pool-comparison',
    )


def faq_list() -> str:
    source_url = DATA['faq_pairs'][0]['source_url']
    return ''.join(
        f'<article class="faq-item" data-faq-pair-id="faq-{index:02d}" data-source-url="{html.escape(source_url, quote=True)}"><details><summary>{html.escape(question)}</summary><p>{html.escape(answer)}</p></details></article>'
...
def build() -> None:
    ...
    outputs={'index.html':home_page(),'about.html':about_page(),'services.html':services_page(),'faq.html':faq_page(),'contact.html':contact_page(),'privacy-policy.html':privacy_page(),'terms-of-service.html':terms_page()}
